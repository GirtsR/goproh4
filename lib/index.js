...
GoPro.Camera = function (options) {
    ...
    this._userAgent = new http.Agent({ localAddress: getCamInterfaceIP() });
...
GoPro.Camera.prototype.restartStream = function () {
    ...
    return this._retryPromise(function (resolve, reject) {
        ...
        request.get({ url: url, insecureHTTPParser: true, timeout: 10 * 1000, agent: self._userAgent }, function (error, response, body) {
            if (!error && response.statusCode === 200) {
                self._keepAliveStart();
                return resolve();
            }
